...
def searchUI(query):
    data = input("Please state first which boolean operator you would like to use to connect your search terms, "
...
def processedQueryUI(query):
    nextStep = input("Please choose your next step:"
...
def chooseDatabaseUI(query):
    nextStep = input("**choose database**"+
        "\n Please choose the database-syntax in which your query should be translated: "
        + "\n *for database1, enter: db1"
        + "\n *for database2, enter: db2"
        + "\n *to exit the python package, type: exit \n")
    if (nextStep == "db1"): translateUI("db1", query)
    elif (nextStep == "db2"): translateUI("db2", query)
    elif (nextStep == "exit"): return
    else:
       print("Error invalid option, try again")
       chooseDatabaseUI(query) 
    return
