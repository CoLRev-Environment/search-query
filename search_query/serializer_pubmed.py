...
def _get_search_field_pubmed(search_field: str) -> str:
...
def _translate_search_fields(query: Query) -> None:
    if query.operator:
        ...
        if query.search_field:
            query.search_field.value = _get_search_field_pubmed(
                query.search_field.value
            )


def _move_field_from_operator_to_terms(query: Query) -> None:
    ...
    for child in query.children:
        ...
        _move_field_from_operator_to_terms(child)


def _combine_tiab(query: Query) -> None:
...
def _flatten_nested_operators(query: Query) -> None:
    ...
    if query.children:
        for child in query.children:
            _flatten_nested_operators(child)
...
def to_string_pubmed(query: Query) -> str:
    ...
    _move_field_from_operator_to_terms(query)
    _flatten_nested_operators(query)
    _combine_tiab(query)
