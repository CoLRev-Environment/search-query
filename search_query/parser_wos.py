...
class WOSMixin:
    """Web of Science class."""

    FIELD_TRANSLATION_MAP = SYNTAX_FIELD_TRANSLATION_MAP[Syntax.WOS]

    def __init__(self, query_str: str, *args, **kwargs) -> None:  # type: ignore
        self.query_str = query_str
        super().__init__(query_str, *args, **kwargs)  # type: ignore
...
class WOSParser(WOSMixin, QueryStringParser):
...
class WOSListParser(WOSMixin, QueryListParser):
    ...
    def is_node_identifier(self, node_content: str) -> bool:
        ...
        return bool(re.match(r"^#\d+$", node_content))
    ...
    def get_children(self, node_content: str) -> list:
        ...
        return re.findall(r"(#\d+|[A-Z]+=\(.*\))", node_content)
