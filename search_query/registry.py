...
class _Registry:
    ...
    list_parsers: Dict[str, Dict[str, Type[QueryListParser]]] = field(default_factory=dict)
    # Serializers
    serializers: Dict[str, Dict[str, Type[QueryStringSerializer]]] = field(default_factory=dict)
    list_serializers: Dict[str, Dict[str, Type[QueryListSerializer]]] = field(default_factory=dict)
    # Translators
    translators: Dict[str, Dict[str, Type[QueryTranslator]]] = field(default_factory=dict)

    # --- register methods (fail on duplicates) ---
    def register_parser_string(self, platform: str, version: str, cls: Type[QueryStringParser]) -> None:
        self.parsers.setdefault(platform, {})
        if version in self.parsers[platform]:
            prev = self.parsers[platform][version]
            raise RuntimeError(f"Duplicate string parser for {platform} {version}: {prev.__name__} vs {cls.__name__}")
        self.parsers[platform][version] = cls  # type: ignore[assignment]

    def register_parser_list(self, platform: str, version: str, cls: Type[QueryListParser]) -> None:
        self.list_parsers.setdefault(platform, {})
        if version in self.list_parsers[platform]:
            prev = self.list_parsers[platform][version]
            raise RuntimeError(f"Duplicate list parser for {platform} {version}: {prev.__name__} vs {cls.__name__}")
        self.list_parsers[platform][version] = cls  # type: ignore[assignment]

    def register_serializer_string(self, platform: str, version: str, cls: Type[QueryStringSerializer]) -> None:
        self.serializers.setdefault(platform, {})
        if version in self.serializers[platform]:
            prev = self.serializers[platform][version]
            raise RuntimeError(f"Duplicate string serializer for {platform} {version}: {prev.__name__} vs {cls.__name__}")
        self.serializers[platform][version] = cls  # type: ignore[assignment]

    def register_serializer_list(self, platform: str, version: str, cls: Type[QueryListSerializer]) -> None:
        ...
        if version in self.list_serializers[platform]:
            ...
            raise RuntimeError(f"Duplicate list serializer for {platform} {version}: {prev.__name__} vs {cls.__name__}")
        self.list_serializers[platform][version] = cls  # type: ignore[assignment]

    def register_translator(self, platform: str, version: str, cls: Type[QueryTranslator]) -> None:
        self.translators.setdefault(platform, {})
        if version in self.translators[platform]:
            prev = self.translators[platform][version]
            raise RuntimeError(f"Duplicate translator for {platform} {version}: {prev.__name__} vs {cls.__name__}")
        self.translators[platform][version] = cls  # type: ignore[assignment]

# Public registries ------------------------------------------------------------

if _t.TYPE_CHECKING:  # pragma: no cover
    from search_query.parser_base import QueryListParser as _QLP
    from search_query.parser_base import QueryStringParser as _QSP
    from search_query.serializer_base import StringSerializer as _QSS
    from search_query.serializer_base import StringSerializer as _QLS
    from search_query.translator_base import QueryTranslator as _QT
...
def _check_pair(name_a: str, m_a: dict[str, dict[str, object]], name_b: str, m_b: dict[str, dict[str, object]], platform: str) -> None:
...
for plat in (p.value for p in PLATFORM):
    ...
    if PARSERS[plat] or LIST_PARSERS[plat]:
        _check_pair("PARSERS", PARSERS, "LIST_PARSERS", LIST_PARSERS, plat)
    if SERIALIZERS[plat] or LIST_SERIALIZERS[plat]:
        _check_pair("SERIALIZERS", SERIALIZERS, "LIST_SERIALIZERS", LIST_SERIALIZERS, plat)
...
LATEST_SERIALIZERS: dict[str, type[QueryStringSerializer]] = {
    k: SERIALIZERS[k][LATEST_VERSIONS_SERIALIZERS[k]] for k in LATEST_VERSIONS_SERIALIZERS
}
LATEST_LIST_SERIALIZERS: dict[str, type[QueryListSerializer]] = {
    k: LIST_SERIALIZERS[k][LATEST_VERSIONS_SERIALIZERS[k]] for k in LATEST_VERSIONS_SERIALIZERS
}

LATEST_VERSIONS_TRANSLATORS: dict[str, str] = _latest_map(TRANSLATORS)
LATEST_TRANSLATORS: dict[str, type[QueryTranslator]] = {
    k: TRANSLATORS[k][LATEST_VERSIONS_TRANSLATORS[k]] for k in LATEST_VERSIONS_TRANSLATORS
}
